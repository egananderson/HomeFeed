...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        self.contentImage.frame = CGRectMake(0, 0, 0, 0);
...
- (void)setUpCell:(NSDictionary *)contents{
    self.post = [[BasicPost alloc] initWithImage:contents[@"contentImage"] contentText:contents[@"contentText"] posterName:contents[@"posterName"] posterImage:contents[@"posterImage"] postID:contents[@"posterID"] numComments:contents[@"numComments"] postType:contents[@"postType"]];
    ...
    //    self.contentImage.image = self.post.contentImage;
    //    [self.contentImage constrainWidthToView:self.postView predicate:@"-20"];
    //    [self.contentImage constrainAspectRatio:@"1"];
    
    //if the contentImage is a blank image
    if (self.post.contentImage.size.width == 0) {
        self.contentImage = nil;
        [self.contentImage constrainTopSpaceToView:self.contentText predicate:@"0"];
        [self.contentImage constrainHeight:@"0"];
        [self.contentImage constrainWidth:@"0"];
    } else {
        self.contentImage.image = self.post.contentImage;
        [self.contentImage constrainWidthToView:self.postView predicate:@"-20"];
        [self.contentImage constrainAspectRatio:@"1"];
